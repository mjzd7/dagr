...
impl LocalIndexStore {
    ...
    pub fn is_file_cached(&self, file_path: &str, current_blake3_hash: &str) -> Result<bool> {
        let mut stmt = self
            .conn
            .prepare("SELECT blake3_hash FROM file_cache WHERE file_path = ?1")?;

        let cached_hash: rusqlite::Result<String> =
...
mod tests {
    ...
    fn test_sqlite_caching_and_symbol_store() -> Result<()> {
        ...
        assert!(!store.is_file_cached(file_path, hash)?);
        store.update_file_cache(file_path, hash)?;
        assert!(store.is_file_cached(file_path, hash)?);
        assert!(!store.is_file_cached(file_path, "different_hash")?);
        ...
        store.store_symbols(file_path, std::slice::from_ref(&node))?;
