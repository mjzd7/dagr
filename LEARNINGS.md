...
- **L7.1 (W2)** The env's OPENAI_API_KEY is a freellmapi gateway key that the gateway itself rejects (401 all auth styles) — G1 blocked on credentials, not code. Provider improvements landed anyway: OPENAI_BASE_URL support, loud API-error surfacing (silent empty-text grading masked the 401 initially — lesson: eval harnesses must fail loudly), markdown fence stripping.
- **L8.1 (G6)** base_candidates must return WORKSPACE-RELATIVE paths — ws.join() on AliasMap candidates (already relative) made them absolute so they never matched git's relative deleted-set. Path-domain consistency is the recurring bug class in this codebase.
- **L8.2 (G6)** TS export wrappers hide definitions: `export const x` → lexical_declaration (name on variable_declarator child) inside export_statement. ReverseIndex now unwraps one level AND skips the inner node's duplicate registration via parent check.
- **L8.3 (G5)** LSP over stdio: frame with Content-Length byte count, skip notifications while waiting for a response id. DAGR_LSP_BIN/DAGR_LSP_ARGS env overrides make the bridge testable without rust-analyzer installed. Deleted-file references work by didOpen'ing virtual content.
- **L9.1 (W4)** Real-world SPDX in the wild: "(MIT OR Apache-2.0) AND Unicode-3.0", "Apache-2.0 WITH LLVM-exception", "MIT/Apache-2.0" — naive split-on-OR is wrong. 60-line recursive-descent evaluator (parens, AND=all-branches, WITH exceptions, + suffixes) covers the ecosystem's actual shapes. unicode-ident is the canonical AND-case.
- **L9.2** Shell workdir leaks between bash calls when workdir param omitted — caused two phantom failures (patch applied to nonexistent path; build from wrong root). Set workdir explicitly every call.
- **L10.1** OpenRouter integration: OPENAI_BASE_URL=https://openrouter.ai/api/v1; `openrouter/free` auto-routing tier survives where named free models were saturated with 429s; retry w/ linear backoff is mandatory on free tiers.
- **L10.2** The single highest-value practice of the day: upgrading LOCAL clippy to CI's version converted N slow push-watch-fix cycles into one local sweep. Always pin/reproduce CI toolchain locally before pushing to a -D warnings gate.
- **L10.3** Windows path bugs are systemic when git (/) meets std::path (\): fix by canonicalizing at creation points (canon()), never at comparison points. Three separate latent bugs shared this root cause.
